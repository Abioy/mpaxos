...
do
    ...
    do
    #    echo "LAUNCHING DAEMON $i"
    #    command="screen -m -d /bin/bash -c \"~/bworkspace/mpaxos/bin/bench_mpaxos.out -c ~/bworkspace/mpaxos/config/config.beaker/config.$N_HOST.$i -s $n_tosend -g $n_group -d 0 -e $sz_data -q 1 >& ~/bworkspace/mpaxos/script/result.rspaxos.thp/$sz_data.$i\"" 
    #    echo $command
    #    nohup ssh $USER@${MHOST[$i]} $command &
