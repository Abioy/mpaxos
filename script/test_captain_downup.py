...
if os.path.isdir('../results/downup_result_%s'%tag): 
    pass 
else: 
    os.mkdir('../results/downup_result_%s'%tag)
...
for i in range(all_times) :
    ...
    os.system('../bin/test_captain_random_downup %s %s > ../results/downup_result_%s/node%s_times%s_round%s'%(node_num, total_times, tag, node_num, total_times, all_times))
