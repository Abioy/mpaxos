...
for sz_data in 1000 4000 8000 16000 64000 256000 1024000 4096000 8192000 16000000
do
    ...
    #    echo $command
    #    nohup ssh $USER@${MHOST[$i]} $command &
        ~/bworkspace/mpaxos/bin/bench_mpaxos.out -c ~/bworkspace/mpaxos/config/config.beaker/config.$N_HOST.$i -s $n_tosend -g $n_group -d 0 -q 1 >& ~/bworkspace/mpaxos/script/result.rspaxos.lat/$sz_data.$i 
    done
